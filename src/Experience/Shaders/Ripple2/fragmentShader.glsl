...
float sdfBox(in vec2 p, in float R)
{
...
void main()
{
    ...
    // vec2 modUv = vec2(vUv.x - 0.5, vUv.y - 0.5);

    vec3 color = vec3(sdfBox(vUv, USdfMod));
    vec3 color2 = vec3(RayMarch(vec3(0,0,1.0), vec3(vUv, 0.0)));
